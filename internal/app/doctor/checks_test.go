...
func TestBuildChecksNilCallbacks(t *testing.T) {
	...
	if checks := BuildChecks(p, nil, func(command string) bool { return true }); checks != nil {
		t.Fatalf("expected nil checks when resolver is nil, got %v", checks)
	}

	if checks := BuildChecks(p, func(command string) (string, string, string, bool) { return command, command, "fix", true }, nil); checks != nil {
